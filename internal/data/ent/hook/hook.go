...
// The DormFunc type is an adapter to allow the use of ordinary
// function as Dorm mutator.
type DormFunc func(context.Context, *ent.DormMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f DormFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.DormMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.DormMutation", m)
}
