...
func (s *ExportStuLogUseCase) ExportStudent(ctx context.Context, stuLogs []*ent.Student) (string, error) {
	_ = os.MkdirAll("./resource/upload/tmp/", 0775)
	// 创建新的 Excel 文件
	f := excelize.NewFile()
	defer func() {
	...
	headers := []string{"序号", "姓名", "性别", "学号", "年级", "班级", "分数", "宿舍楼", "宿舍号"}
	for i, header := range headers {
		cell := fmt.Sprintf("%c%d", 'A'+i, 1)
		_ = f.SetCellValue("Sheet1", cell, header)
	}
	...
	// 设置行高为57（包括表头和数据行）
	for i := 1; i < len(stuLogs)+1; i++ { // 修正：调整循环范围
		_ = f.SetRowHeight("Sheet1", i+1, 20)
	}
	...
	// 调整特定列的宽度以适应内容
	_ = f.SetColWidth("Sheet1", "A", "A", 8)  // 序号
	_ = f.SetColWidth("Sheet1", "B", "B", 10) // 学生
	_ = f.SetColWidth("Sheet1", "C", "C", 8)  // 性别
	_ = f.SetColWidth("Sheet1", "D", "D", 10) // 学号
	_ = f.SetColWidth("Sheet1", "E", "E", 8)  // 年级
	_ = f.SetColWidth("Sheet1", "F", "F", 8)  // 班级
	_ = f.SetColWidth("Sheet1", "G", "G", 8)  // 分数
	_ = f.SetColWidth("Sheet1", "H", "H", 10) // 宿舍楼
	_ = f.SetColWidth("Sheet1", "I", "I", 10) // 宿舍号

	// 写入每条日志数据
	for i, log := range stuLogs {
		row := i + 2                                               // 第二行开始写数据
		_ = f.SetCellValue("Sheet1", fmt.Sprintf("A%d", row), i+1) // 事件编号
		_ = f.SetCellValue("Sheet1", fmt.Sprintf("B%d", row), log.Name)
		_ = f.SetCellValue("Sheet1", fmt.Sprintf("C%d", row), log.Sex)
		_ = f.SetCellValue("Sheet1", fmt.Sprintf("D%d", row), log.StuNum)
		_ = f.SetCellValue("Sheet1", fmt.Sprintf("E%d", row), log.Edges.Grade.GradeName)
		_ = f.SetCellValue("Sheet1", fmt.Sprintf("F%d", row), log.Edges.Class.ClassName)
		_ = f.SetCellValue("Sheet1", fmt.Sprintf("G%d", row), log.Score)
		if log.Edges.Dorm != nil {
			_ = f.SetCellValue("Sheet1", fmt.Sprintf("H%d", row), log.Edges.Dorm.Building) // 宿舍（楼栋）
			_ = f.SetCellValue("Sheet1", fmt.Sprintf("I%d", row), log.Edges.Dorm.DormNum)  // 宿舍（房间号）
		}
	}
	// 设置默认工作表
	f.SetActiveSheet(index)
	...
	// 保存文件
	err = f.SaveAs("./resource/upload/tmp/stu_export.xlsx")
	if err != nil {
		fmt.Println("Error saving file:", err)
		return "", err
	}
	return "/resource/upload/tmp/stu_export.xlsx", err
}
